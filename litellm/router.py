...
class Router:
    ...
    def init_adept_router_deployment(self, deployment: Deployment) -> None:
        ...
        # Idempotency + refresh-on-change:
        #
        # This method is called every 30s by the DB-sync loop. If the same
        # model_name is already registered AND its params match the current row,
        # do nothing. If a param changed (e.g. the operator edited trainer_url
        # via the UI), tear down and rebuild the in-memory router so the new
        # value takes effect without requiring a proxy restart.
    ...
    def _set_model_group_info(  # noqa: PLR0915
        ...
        for model in model_list:
            ...
            # ADEPT models route through a custom strategy, not a standard provider.
            # Skip get_llm_provider — it would raise "Unsupported provider" and pollute
            # the error log every time the UI fetches model-group info.
            if self._is_adept_router_deployment(litellm_params=litellm_params):
